...
var scroll_3D = (function(require, exports) {
    ...
    move: function() {
      ...
      /* style.transform =
                style.webkitTransform =
                    'perspective(900px) translate3d(' + (this.x + CX - elem.offsetWidth / 2).toFixed(2)  + 'px, ' +
                        (this.y + CY - elem.offsetHeight / 2).toFixed(2) + 'px)' + '  scale(' + scale.toFixed(2) + ')'; */
      style.transform = `perspective(900px) translate3d(${
          (this.x + CX -elem.offsetWidth / 2).toFixed(2)
        }px, ${
            (this.y + CY - elem.offsetHeight / 2).toFixed(2)
        }px, ${this.z + RADIUS}px)`;
      // 触发滚动事件
  ...
  var start = function() {
    let x = Math.abs(angleX) * 0.01,
      y = Math.abs(angleY) * 0.01;
    var go = function() {
      angleX = angleX - x * Math.sign(angleX);
      angleY = angleY - y * Math.sign(angleY);
      if (Math.abs(angleX) < x) {
        return false;
      }
      rotate();
